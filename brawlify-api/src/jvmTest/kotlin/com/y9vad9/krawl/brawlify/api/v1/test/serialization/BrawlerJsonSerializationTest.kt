...
class BrawlerJsonSerializationTest : com.y9vad9.krawl.test.WithJsonFixturesTest() {
    @Test
    fun `check no errors when serializing gamemodes from fixtures`() {
        // GIVEN
        val filePath = "v1/brawlers/multiple.json"
        ...
        assertDoesNotThrow(
            ...
            json.decodeFromStream<ItemsResponse<RawBrawlifyBrawler>>(stream)
        }
    }
    ...
    fun `check no errors when serializing single gamemode from fixtures`() {
        // GIVEN
        val filePath = "v1/brawlers/single.json"
        val stream = loadFixtureAsStream(filePath)
        ...
        assertDoesNotThrow(
            ...
            json.decodeFromStream<RawBrawlifyBrawler>(stream)
        }
    }
}
